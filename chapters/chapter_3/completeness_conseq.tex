...
Being completely honest, Proving $\mathbb{R}$ uniqueness drained me completely, so it is fair to just check a few smaller extensions (consequence of this proof) to see how the visited properties are used in other, more specific lemmas. They also use cool last names to name them, so we can brag about knowing the \textit{Weierstrass-Cauchy-DiCaprio-Cumberbatch principle of boiling water in $\mathbb{R}^6$}. It is also a great excuse to give some color to the notes with some diagrams.
...
\input{assets/nested_seq_diagram.tex}
...
Finally, take special care of how \textit{for all} $a_k, b_k$ we found that there exists a real number $c$ that satisfies this inequality, meaning that $c$ belongs to any interval $I_k$ in the nested sequence.
