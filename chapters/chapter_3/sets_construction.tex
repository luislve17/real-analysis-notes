...
\input{assets/real-numbers-diagram.tex}

From this idea, we need to build $\mathbb{N_F}$, $\mathbb{Z_F}$, $\mathbb{Q_F}$ and $\mathbb{I_F}$ to finally get this $\mathbb{F}$ we want. This will all make sense in the last part of the process, and it is true that it will be a bit of a hustle to construct all of it (specially $\mathbb{I_F}$ from $\mathbb{Q_F}$), but we need to go step by step.


\begin{definition}[$\mathbb{N}_{\mathbb{F}}$ by induction]
Let $\mathbb{F}$ be \textit{the} complete ordered field. Using $1_\mathbb{F}$: multiplicative neutral of $\mathbb{F}$, we declare the following element construction:
...
Notice how we only used existent elements from $\mathbb{F}$ computed using the field's operations and nothing else. It is important to mention that you as the reader don't know yet the usage of these new sets, although we could make a good guess.

Now comes the tricky part for us. One could think that the next steps involve 1. to prove that the irrational (in $\mathbb{F}$) is in fact buildable from $\mathbb{Q_F}$, and 2. to prove that there is an isomorphism between this result and the $\mathbb{R}$ we know.
