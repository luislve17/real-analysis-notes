...
\begin{solution}
Let $s := \sup X = \inf Y$ (by the previous problem), and since $X \cup Y = \mathbb{R}$, and $s \in \mathbb{R}$, we must have $s \in X$ or $s \in Y$.

\textbf{Case 1: $s \in X$.}

Since $s = \sup X$ is an upper bound of $X$, and $s \in X$:
\begin{align*}
\forall x \in X, \ x \leq s \quad \wedge \quad s \in X \Rightarrow s = \max X
\end{align*}

\textbf{Case 2: $s \in Y$.}

Since $s = \inf Y$ is a lower bound of $Y$, and $s \in Y$:
\begin{align*}
\forall y \in Y, \ s \leq y \quad \wedge \quad s \in Y \Rightarrow s = \min Y
\end{align*}

$$\therefore X \text{ has a maximal element, or } Y \text{ has a minimal element.}$$
\end{solution}
...
\begin{solution}
Let $s := \sup{(-A)}$, so that $\forall -a \in A: -a \leq s$.
\begin{align*}
\forall -a \in A: -a \leq s \\
\rightarrow -(-a) \geq -s \\
\rightarrow -s \leq a \\
\rightarrow -s : \text{lower bound of } A
\end{align*}

Additionally, for $-A$ we have by the supremum's characterization:
\begin{align*}
\forall \epsilon > 0, &\exists x \in -A: s - \epsilon < x \leq s\\
(\text{mul.} -1)&: -s + \epsilon > -x \geq -s, \ -x \in A\\
(\text{fixing ineq. side})&: -s \leq -x < -s + \epsilon \\
\Rightarrow -s&: \text{greatest lower bound of } A \\
\end{align*}
