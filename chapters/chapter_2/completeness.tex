...
A set $X \subset \mathbb{F}$ ($\mathbb{F}$ ordered field) is said to be \textit{bounded above} (or respectively, \textit{bounded below}) if $\exists \ c \in \mathbb{F}$ such that $\forall a \in X, a \leq c$ (or respectively, $c \leq a$). $c$ is called upper (or respectively lower) bound of $X$.
...
Now, given this definition, it is important to notice that not every set, not even every bounded set, has a maximal or minimal element. For example:
$$
...
\begin{align*}
(s = \sup X) := \left(\forall x \in X, \ x \leq s\right) \ \wedge \ \left(\forall s' < s, \ \exists x' \in X, \ s' < x'\right)
\end{align*}
\end{definition}
...
\begin{itemize}
\item $\forall x \in X$: The following definition applies to every element in $X$.
...
\item $(s^{\prime} < x^{\prime})$: It is larger than the $s^{\prime}$, showing that $s^{\prime}$ is not an upper bound.
\end{itemize}
\end{itemize}
...
\begin{align*}
(i = \inf X) := \left(\forall x \in X, \ i \leq x\right) \ \wedge \ \left(\forall i' > i, \ \exists x' \in X, \ x' < i'\right)
\end{align*}

\end{definition}

Thus, we have now the following properties from the definitions:
...
\begin{theorem}[Characterization of the Supremum]\label{th:sup-characterization}
Let $X \subset \mathbb{F}$ be a nonempty set in an ordered field $\mathbb{F}$, and let $s \in \mathbb{F}$. Then $s = \sup X$ if and only if
$$
\forall x \in X: x \leq s \land \forall \epsilon > 0,\ \exists x \in X: s - \epsilon < x.
$$
\end{theorem}

\begin{bookproof}
Suppose $s = \sup X$. By definition, $s$ is an upper bound of $X$: $$\forall x \in X: x \leq s.$$

Also, by the definition of the least upper bound, no $s' < s$ is an upper bound of $X$. Therefore: $$\forall s' < s,\ \exists x \in X: s' < x.$$

Let $\epsilon > 0$ and take $s' = s - \epsilon$. Since $\epsilon > 0$: $$s - \epsilon < s.$$

Therefore: $$\exists x \in X: s - \epsilon < x.$$

Conversely, suppose that:
$$\forall x \in X: x \leq s \land \forall \epsilon > 0,\ \exists x \in X: s - \epsilon < x.$$

The first condition shows that $s$ is an upper bound of $X$.

Now, let $s' < s$. Then: $$\epsilon := s - s' > 0.$$

By assumption: $$\exists x \in X: s - \epsilon < x.$$

Since $\epsilon = s - s'$:$$s' < x.$$

Therefore, $s'$ is not an upper bound of $X$. Hence, no number smaller than $s$ is an upper bound of $X$, so $s$ is the least upper bound of $X$.
...
\begin{example}\label{ex:s-in-q-not-bounded}
Consider the ordered field $\mathbb{Q}$ of rational numbers, and let:
$$S = \{x \in \mathbb{Q} \ | \ x^2 < 2\}$$

The set $S$ is nonempty (for instance, $1 \in S$) and bounded above (for instance, $2 \in \mathbb{F}$ is an upper bound). However, $S$ has no supremum in $\mathbb{Q}$:  if such a supremum existed, it would equal $\sqrt{2} \notin \mathbb{Q}$, so $S$ has no least upper bound within the rational numbers.
...
\begin{theorem}[Least Upper Bound Principle]\label{th:lub-principle}
