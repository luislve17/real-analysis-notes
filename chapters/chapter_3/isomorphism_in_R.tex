...
\begin{bookproof}
First, let us check the definition:
$$\Phi(\alpha) := \sup\{\phi(q): q \in \mathbb{Q}, q < \alpha \}$$
...
\end{align*}
Let us denote $\epsilon_M$ as the distance between this assumed $M$ and $\phi(r)$.

Now, let us not forget that by density in $\mathbb{Q}$ we are able to locate an arbitrary rational number between two other arbitrary rationals, or even better for our use case: we can position an arbitrary rational $q^\prime$ as close as we want to any other arbitrary rational.
...
Taking the time to prove this mapping for rationals wasn't a waste of time after all, since it confirms that our intuition about the definition of $\Phi$ isn't arbitrary, but respects the structure we already built.
...
\begin{bookproof}
To prove this, what matters the most is that $S$ is well-defined in $\mathbb{F}$. That automatically guarantees that $\Phi$ makes sense in $\mathbb{R}$.
\begin{enumerate}
...
$S(\alpha) = \{\phi(q) : q \in \mathbb{Q} \text{ AND } q < \alpha\}$. The constraint $q < \alpha$ obliges us to prove that \textit{there exists at least one rational $q$ with $q < \alpha$}.
...
\begin{bookproof}
let us start with two evaluations of set $S$:
\begin{itemize}
...
Now comes the final sprint to prove that $\Phi$ maps $\mathbb{F}$ and $\mathbb{R}$ perfectly. Just as we did with $\phi$, we need to prove isomorphism by proving bijection and homomorphism. From this last part, the only tricky step is for surjectiveness, since it uses a method that we haven't seeing before. We are almost there!
...
\begin{bookproof}
Let us support the following ideas on the next diagram:
...
Now let us consider $\alpha = \sup(A)$. We first show that $\alpha$ exists.
...
Now that this is done, we can conclude that $\Phi$ is a bijection. Next we will just cover the requirements to prove homomorphism and finish the whole demonstration. But first, a handy property:
...
\begin{lemma}[Additivity of $\Phi$]\label{le:Phi-additive}
$\Phi: \mathbb{R} \rightarrow \mathbb{F}$ preserves addition:
$$\Phi(\alpha + \beta) = \Phi(\alpha) + \Phi(\beta), \quad \forall \alpha, \beta \in \mathbb{R}$$
\end{lemma}
...
\begin{lemma}[Multiplicativity of $\Phi$]\label{le:Phi-multiplicative}
$\Phi: \mathbb{R} \rightarrow \mathbb{F}$ preserves multiplication:
$$\Phi(\alpha \cdot \beta) = \Phi(\alpha) \cdot \Phi(\beta), \quad \forall \alpha, \beta \in \mathbb{R}$$
\end{lemma}

\begin{bookproof}
Before splitting into cases, note a consequence of \cref{le:Phi-additive}: since $\Phi$ preserves addition,
$$\Phi(\alpha)+\Phi(-\alpha) = \Phi(\alpha + (-\alpha)) = \Phi(0)$$
Taking $\alpha = 0$ gives $\Phi(0) = 0_\mathbb{F}$, and substituting back:
$$\Phi(-\alpha) = -\Phi(\alpha), \quad \forall \alpha \in \mathbb{R}$$
We will use this freely below.

\textbf{Case 1: $\alpha = 0$ or $\beta = 0$.} Then $\alpha\beta = 0$, and
$$\Phi(\alpha)\Phi(\beta) = 0_\mathbb{F} = \Phi(0) = \Phi(\alpha\beta)$$

\textbf{Case 2: $\alpha, \beta > 0$.}\\

\textbf{i. $\Phi(\alpha)\Phi(\beta)$: Upper Bound for $S(\alpha\beta)$}

Let us take any $\phi(q) \in S(\alpha\beta)$ where $q < \alpha\beta$.

If $q \leq 0$: since $\alpha, \beta > 0$, density of $\mathbb{Q}$ in $\mathbb{R}$ lets us pick positive rationals $q_1 < \alpha$ and $q_2 < \beta$. Then $q_1 q_2 > 0 \geq q$, so
$$\phi(q) \leq \phi(q_1 q_2) = \phi(q_1)\phi(q_2)$$

If $q > 0$: since $\beta > 0$ and $q < \alpha\beta$, dividing by $\beta$ gives $\dfrac{q}{\beta} < \alpha$. By density of $\mathbb{Q}$ in $\mathbb{R}$, we can locate a rational squeezed in between:
$$\frac{q}{\beta} < q_1 < \alpha, \quad q_1 \in \mathbb{Q}$$
Since $q_1 > q/\beta > 0$, dividing by $q_1$ gives $\dfrac{q}{q_1} < \beta$. Applying density once more:
$$\frac{q}{q_1} < q_2 < \beta, \quad q_2 \in \mathbb{Q}$$
Rearranging $q_2 > q/q_1$ we get $q < q_1 q_2$, with $q_1 < \alpha$ and $q_2 < \beta$ (and both may be taken positive, since $\alpha, \beta > 0$). By order preservation of $\phi$:
$$\phi(q) < \phi(q_1 q_2) = \phi(q_1)\phi(q_2)$$

In either case, we land on rationals $q_1 < \alpha$ and $q_2 < \beta$ with $\phi(q) \leq \phi(q_1)\phi(q_2)$.
...
Since $q_1, q_2$ were taken positive, multiplying these two inequalities preserves their direction:
$$\phi(q_1) \cdot \phi(q_2) \leq \Phi(\alpha) \cdot \Phi(\beta)$$

Chaining everything together:
$$\phi(q) \leq \phi(q_1) \cdot \phi(q_2) \leq \Phi(\alpha) \cdot \Phi(\beta)$$
...
We need to prove that for any $q \in S(\alpha . \beta)$, it will be bounded by $\Phi(\alpha) \cdot \Phi(\beta)$. This is equivalent to prove that there is an $\epsilon _\mathbb{F} > 0$, the smallest of them all, that makes $\phi(q) + \epsilon _\mathbb{F} > \Phi(\alpha) \cdot \Phi(\beta)$.
...
$$
Knowing this, our $\delta$ must be a positive number in $\mathbb{Q}$. So let us define it as
$$
...
let us leave the inequality there to see more deeply what we want to achieve using $\delta$. The intuition here is to find a value for $\delta$ that allows the inequality to hold, no matter what.
...
Now allow me to do a \textquotedblleft cleanup\textquotedblright. let us redefine $\delta$ (still between $0$ and $1$) as:
...
$$\Rightarro2 \Phi(\alpha . \beta) = \sup\{\phi(q),q \in \mathbb{Q}, q < \alpha.\beta\} = \Phi(\alpha) \cdot \Phi(\beta), \quad \forall \alpha, \beta > 0$$

\textbf{Case 3: $\alpha, \beta < 0$.} Then $-\alpha, -\beta > 0$, so by the case just proven:
$$\Phi((-\alpha)(-\beta)) = \Phi(-\alpha)\Phi(-\beta) = (-\Phi(\alpha))(-\Phi(\beta)) = \Phi(\alpha)\Phi(\beta)$$
Since $(-\alpha)(-\beta) = \alpha\beta$, this gives $\Phi(\alpha\beta) = \Phi(\alpha)\Phi(\beta)$.\\

\textbf{Case 4: $\alpha > 0, \beta < 0$} (the case $\alpha < 0, \beta > 0$ is symmetric):\\

Write $\beta = -\gamma$ with $\gamma > 0$. Then $\alpha\beta = -(\alpha\gamma)$, and by the case $\alpha, \gamma > 0$ proven above:
$$\Phi(\alpha\gamma) = \Phi(\alpha)\Phi(\gamma)$$
So:
$$\Phi(\alpha\beta) = \Phi(-(\alpha\gamma)) = -\Phi(\alpha\gamma) = -\Phi(\alpha)\Phi(\gamma) = \Phi(\alpha)(-\Phi(\gamma)) = \Phi(\alpha)\Phi(-\gamma) = \Phi(\alpha)\Phi(\beta)$$

$$\Rightarrow \Phi(\alpha\beta) = \Phi(\alpha)\Phi(\beta), \quad \forall \alpha, \beta \in \mathbb{R}$$
\end{bookproof}
...
\begin{bookproof}
\begin{enumerate}
	\item By \cref{le:Phi-additive}, $\Phi$ preserves addition.
	\item By \cref{le:Phi-multiplicative}, $\Phi$ preserves multiplication.
\end{enumerate}
...
Personally, this is the first time I have been this rigorous with a proof, covering as much detail as I felt necessary to reach the result. For me, the real reward was the joy of knowing I understood all the pieces needed to build the puzzle, and I hope you found it equally rewarding.
